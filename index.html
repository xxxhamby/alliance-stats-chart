...
<body>
    ...
    <script type="module">
        ...
        import { getFirestore, collection, doc, getDoc, getDocs, setDoc, updateDoc, deleteDoc, serverTimestamp, query, orderBy, limit, where, startAt, endAt, getCountFromServer, addDoc, writeBatch, onSnapshot, arrayUnion, arrayRemove } from "https://www.gstatic.com/firebasejs/10.12.2/firebase-firestore.js";
        ...
        const State = {
            ...
            data: { global: [], alliance: [], roster: [], freelancers: [], history: [], logs: [] },
            ...
            config: { lockdown: false, accessCode: '', creationCode: '', hideGlobal: false },
        ...
        const App = {
                        ...
                        if (!State.listeners.security) {
                            State.listeners.security = onSnapshot(doc(App.db, 'metadata', 'security'), (snap) => {
                                if(snap.exists()) { 
                                    ...
                                    const hasProfile = State.profile && State.profile.ign;
                                    const isOwner = State.dbUser?.role === 'owner';
                                    
                                    if(State.config.lockdown && !isOwner && !hasProfile && State.current !== 'locked') {
                                        State.current = 'locked'; 
                                        App.render();
                                    }
                                    
                                    if(State.current === 'owner') App.render();
                                }
                            });
                        }

                        // INITIAL LOCKDOWN CHECK
                        const configSnap = await getDoc(doc(App.db, 'metadata', 'security'));
                        if(configSnap.exists()) State.config = configSnap.data();

                        const isOwner = State.dbUser?.role === 'owner';

                        if (!profileSnap.exists()) {
                            if (State.config.lockdown && !isOwner) {
                                State.current = 'locked';
                            } else {
            ...
            // --- PERMISSION HELPERS ---
            permissions: {
                isOwner: () => State.dbUser?.role === 'owner',
                isLeader: () => State.alliance && State.alliance.leader === State.user?.uid,
                isManager: () => State.alliance && (State.alliance.leader === State.user?.uid || State.alliance.managers?.includes(State.user?.uid)),
                
                canEdit: (targetUid) => {
                    if (!State.user) return false;
                    const me = State.user.uid;
                    if (App.permissions.isOwner()) return true;
                    if (me === targetUid) return true;
                    if (!State.alliance) return false;
                    
                    const targetInAlliance = State.data.roster.some(m => m.id === targetUid);
                    if (!targetInAlliance) return false;

                    const targetIsLeader = State.alliance.leader === targetUid;
                    if (App.permissions.isLeader()) return !targetIsLeader; 
                    if (App.permissions.isManager()) return !targetIsLeader && !State.alliance.managers?.includes(targetUid);
                    
                    return false;
                },

                canViewHidden: (target) => {
                    if (!State.user) return false;
                    if (App.permissions.isOwner()) return true;
                    if (State.user.uid === target.id) return true;
                    if (!State.alliance) return false;
                    
                    if (target.allianceID === State.alliance.id) {
                        if (App.permissions.isManager()) return true; 
                    }
                    return false;
                }
            },
            ...
            render: () => {
                if (State.current === State.lastView && !['view_global', 'view_alliance', 'allianceManager', 'owner'].includes(State.current)) return;

                const allianceBtnText = State.profile?.allianceID ? 'My Alliance' : 'Join Alliance';
                
                const lockdownBanner = State.config.lockdown 
                    ? `<div class="bg-red-600 text-white text-center text-xs font-bold py-1 tracking-widest"><i class="fas fa-lock mr-2"></i>GLOBAL LOCKDOWN ACTIVE</div>` 
                    : '';

                const brandedHeader = `
                    <div class="bg-slate-800 p-4 rounded border-l-4 border-blue-500 mb-6 shadow-lg">
                        <h1 class="text-3xl font-extrabold text-white text-center mb-2 tracking-tight uppercase">Kingshotalytics</h1>
                        <div class="grid grid-cols-2 md:grid-cols-4 gap-4 text-center text-xs md:text-sm text-slate-300 mt-4">
                            <div class="bg-slate-900 p-2 rounded border border-slate-700"><i class="fas fa-ban text-red-500 mb-1 text-lg block"></i>NO BUFFS Active</div>
                            <div class="bg-slate-900 p-2 rounded border border-slate-700"><i class="fas fa-crosshairs text-blue-500 mb-1 text-lg block"></i>Rally Alt (No Troops)</div>
                            <div class="bg-slate-900 p-2 rounded border border-slate-700"><i class="fas fa-chess-pawn text-purple-500 mb-1 text-lg block"></i>4 Chenkos (Lvl 5 SoA)</div>
                            <div class="bg-slate-900 p-2 rounded border border-slate-700"><i class="fas fa-scroll text-green-500 mb-1 text-lg block"></i>Use Battle Report</div>
                        </div>
                    </div>
                `;
                ...
                const footer = `
                    <footer class="p-4 text-center text-xs text-slate-500 mt-auto border-t border-slate-800">
                        ...
                        <a href="https://docs.google.com/document/d/1UD2GrMhZkWcxSCVg5Rfs9i2rx5wUnuzsSOaYqFMAZms/edit?tab=t.0" target="_blank" class="hover:text-blue-400 transition flex items-center justify-center gap-2">
                ...
                const lockedView = `
                    <div class="flex flex-col items-center justify-center h-screen bg-slate-900 p-6">
                         <div class="bg-slate-800 p-12 rounded-lg text-center border-4 border-red-600 shadow-2xl max-w-2xl w-full">
                            <h1 class="text-4xl font-extrabold text-red-500 mb-4 animate-pulse">SYSTEM LOCKED</h1>
                            <p class="text-slate-300 text-lg mb-6">Kingshotalytics is currently in **Blackout Mode** for Kingdom vs Kingdom operations. New registrations are disabled.</p>
                            <img src="https://firebasestorage.googleapis.com/v0/b/alliance-stats-96973.firebasestorage.app/o/daf29c5a-720b-434f-825a-8d3766753036.jpg?alt=media&token=054e403a-b638-4976-aee5-96a79cd9ec52" class="w-full rounded shadow-lg mb-6">
                         </div>
                    </div>
                `;
                    ...
                    view_global: `
                        <div class="min-h-screen bg-slate-900 text-slate-200">
                            ...
                            <div class="p-4 max-w-7xl mx-auto">
                                ${brandedHeader}
                                ${(State.config.lockdown || State.config.hideGlobal) && !App.permissions.isOwner() ? 
                                    `<div class="bg-slate-800 p-12 rounded text-center border border-red-600 shadow-2xl flex flex-col items-center">
                                        <div class="w-full max-w-lg mb-4">
                                            <img src="https://firebasestorage.googleapis.com/v0/b/alliance-stats-96973.firebasestorage.app/o/daf29c5a-720b-434f-825a-8d3766753036.jpg?alt=media&token=054e403a-b638-4976-aee5-96a79cd9ec52" class="w-full rounded-lg shadow-2xl" alt="Lockdown Active">
                                        </div>
                                    </div>` :
            ...
            ui: {
                        ...
                        <div class="bg-slate-800 p-4 rounded border-l-4 border-yellow-500 flex justify-between items-center mb-6">
                            <div class="w-full">
                                <div class="flex justify-between items-start">
                                    ...
                                    <div class="text-right flex flex-col gap-2 items-end">${isLeader ? `<button onclick="window.openAllianceManager()" class="bg-yellow-600 hover:bg-yellow-500 text-white text-xs font-bold px-3 py-2 rounded"><i class="fas fa-users-cog mr-1"></i> MANAGE</button>` : `<button onclick="window.leaveAlliance()" class="bg-red-900/50 hover:bg-red-800 text-red-300 border border-red-800 text-xs font-bold px-3 py-2 rounded">LEAVE</button>`}</div>
                                </div>
                ...
                generateManagerRosterRows: (rosterData) => {
                    return rosterData.map(m => {
                        const isLeader = m.id === State.alliance.leader;
                        const isManager = State.alliance.managers?.includes(m.id);
                        const escapedIgn = m.ign.replace(/'/g, "\\'");
                        
                        let actionBtn = '';
                        if (isLeader) actionBtn = '<span class="text-xs text-yellow-500 font-bold">LEADER</span>';
                        else if (App.permissions.isLeader()) {
                            actionBtn = `
                                ...
                                ${isManager ? 
                                    ...
                                    `<button onclick="event.stopPropagation(); window.promoteMember('${m.id}')" class="text-blue-400 hover:text-blue-300 text-xs font-bold border border-blue-900 bg-blue-900/20 px-2 py-1 rounded ml-1">PROMOTE</button>`
                                }`;
                        } else if (App.permissions.isManager() && !isManager) {
                             actionBtn = `<button onclick="event.stopPropagation(); window.kickMember('${m.id}', '${escapedIgn}')" class="text-red-400 hover:text-red-300 text-xs font-bold border border-red-900 bg-red-900/20 px-2 py-1 rounded">KICK</button>`;
                        }
                        
                        return `
                        <tr class="hover:bg-slate-700 cursor-pointer" onclick="window.viewProfile('${m.id}')">
                            <td class="p-3 font-bold text-white flex items-center gap-2">
                                ${isLeader ? '👑' : (isManager ? '🛡️' : '')} ${m.ign} <span class="text-slate-500 text-xs font-mono">#${m.pid}</span>
                            </td>
                ...
                renderRows: (dataset) => {
                    ...
                    return sorted.map(p => {
                        ...
                        const isHidden = (p.privacy?.hideName || p.privacy?.hideStats) && !App.permissions.canViewHidden(p);
                        
                        const displayName = (p.privacy?.hideName && isHidden) ? `<span class="italic text-slate-500">Classified</span>` : p.ign;
                        ...
                        const cell = (path, val) => {
                            if(isHidden && p.privacy?.hideStats) return '<td class="text-slate-500">?</td>';
                            const oldVal = getValue(prev, path);
                            return `<td class="text-slate-300">${val}${getDelta(val, oldVal)}</td>`;
                        };
                        
                        const canEditTarget = App.permissions.canEdit(p.id);
                        const editBtn = canEditTarget ? `<button onclick="event.stopPropagation(); window.editPlayer('${p.id}')" class="text-blue-500 hover:text-white ml-2" title="Edit"><i class="fas fa-pencil-alt text-xs"></i></button>` : '';

                        // Visual Badge
                        const isLeader = State.alliance && State.alliance.leader === p.id;
                        const isManager = State.alliance && State.alliance.managers?.includes(p.id);
                        let badge = '';
                        if (isLeader) badge = '👑 ';
                        else if (isManager) badge = '🛡️ ';

                        const allianceCol = State.viewMode === 'global' 
                        ...
                        <tr class="hover:bg-slate-800 transition cursor-pointer border-b border-slate-800" onclick="window.viewProfile('${p.id}')">
                            ...
                            <td class="text-white font-bold text-left pl-3 py-2 text-xs flex items-center">${badge}${displayName} ${editBtn}</td>
                ...
                showProfile: (data) => {
                    ...
                    const canView = App.permissions.canViewHidden(data);
                    ...
                    const displayName = (data.privacy?.hideName && !canView) ? "Classified" : data.ign;

                    const renderStatRow = (label, obj, color) => {
                        if (data.privacy?.hideStats && !canView) return `<div class="bg-slate-900 p-3 rounded text-center text-slate-500 italic">Stats Classified</div>`;
                        return `
                ...
                showAnalytics: () => {
                        ...
                        <div class="bg-slate-800 p-6 rounded-lg shadow-2xl max-w-5xl w-full border-2 border-slate-600 modal-enter h-[85vh] flex flex-col">
                            ...
                            <div class="flex-grow grid grid-cols-1 md:grid-cols-3 gap-6 overflow-y-auto">
                                <div class="bg-slate-900 p-4 rounded border border-slate-700">
                                    <h3 class="text-xs font-bold text-blue-400 mb-2 uppercase">Infantry Tech</h3>
                                    <div class="h-32"><canvas id="chart-inf"></canvas></div>
                                </div>
                                <div class="bg-slate-900 p-4 rounded border border-slate-700">
                                    <h3 class="text-xs font-bold text-green-400 mb-2 uppercase">Cavalry Tech</h3>
                                    <div class="h-32"><canvas id="chart-cav"></canvas></div>
                                </div>
                                <div class="bg-slate-900 p-4 rounded border border-slate-700">
                                    <h3 class="text-xs font-bold text-red-400 mb-2 uppercase">Archer Tech</h3>
                                    <div class="h-32"><canvas id="chart-arc"></canvas></div>
                                </div>
                                <div class="bg-slate-900 p-4 rounded border border-slate-700">
                    ...
                    filterSameKingdom.forEach(m => {
                        ...
                        ['inf','cav','arc'].forEach(type => {
                            ...
                            techCounts[type][key] = (techCounts[type][key] || 0) + 1;
                        });
                    });

                    // Weighted Sort Helper
                    const techSort = (a, b) => {
                        const parse = (s) => {
                            const tier = parseInt(s.match(/T(\d+)/)?.[1] || 0);
                            const tg = parseInt(s.match(/TG(\d+)/)?.[1] || 0);
                            return tier * 100 + tg;
                        };
                    ...
                    ['inf', 'cav', 'arc'].forEach(type => {
                         ...
                         if(ctx) {
                             const sortedKeys = Object.keys(techCounts[type]).sort(techSort);
                             new Chart(ctx, {
                                type: 'bar',
                                data: {
                                    labels: sortedKeys,
                                    datasets: [{ label: 'Count', data: sortedKeys.map(k => techCounts[type][k]), backgroundColor: type === 'inf' ? '#3b82f6' : (type === 'cav' ? '#22c55e' : '#ef4444') }]
                                },
            ...
            actions: {
                    ...
                    try {
                        ...
                        if (State.viewMode === 'alliance' && State.profile?.allianceID) {
                            ...
                            if (State.filters.ign) {
                                ...
                                // FIXED: TYPE SAFE QUERY (Number Check)
                                // We first check if input is a valid number
                                const kingdomNum = parseInt(State.filters.kingdom);
                                
                                if (!isNaN(kingdomNum)) {
                                    // Query for Exact Match on Number
                                    q = query(q, where('kingdom', '==', kingdomNum), orderBy('updatedAt', 'desc'), limit(50));
                                } else {
                                    // Fallback for weird input (just load recent)
                                    q = query(q, orderBy('updatedAt', 'desc'), limit(50));
                                }
                            } else {
                                q = query(q, orderBy('updatedAt', 'desc'), limit(500)); 
                            }
                        }
                        const snap = await getDocs(q);
                ...
                submitGate: async () => {
                    // NEW: Force lockout if owner enabled it
                    if(State.config.lockdown && State.dbUser?.role !== 'owner') {
                         return App.ui.toast("SYSTEM LOCKED", "error");
                    }
                ...
                createAlliance: async () => {
                    ...
                    // NEW: Creation Code Check
                    const creationCode = document.getElementById('admin-creation-code')?.value || '';
                    
                    if (State.config.creationCode && creationCode !== State.config.creationCode) {
                         return App.ui.toast("Invalid Creation Code", "error");
                    }
                ...
                deleteAlliance: async () => {
                    ...
                    if (State.alliance.leader !== State.user.uid && State.dbUser?.role !== 'owner') {
                         return alert("PERMISSION ERROR: Only the Leader or Owner can delete the alliance. If you are the leader, update your Firestore Rules.");
                    }
                    ...
                    try {
                        ...
                        console.error(e); 
                        alert("Delete Failed. Check Console for Permission Error."); 
                    }
                },
                ...
                rotateAllianceCode: async () => {
                    ...
                    // Force Re-render to show new code
                ...
                // NEW: Promote/Demote
                promoteMember: async (uid) => {
                    if (!confirm(`Promote this user to Manager?`)) return; // FIXED: No variable dependency
                    try {
                        // Update Alliance Doc array
                        await updateDoc(doc(App.db, 'alliances', State.alliance.id), {
                        ...
                        await addDoc(collection(App.db, 'alliances', State.alliance.id, 'logs'), {
                            action: 'PROMOTION',
                            details: `${target ? target.ign : uid} promoted to Manager by ${State.profile.ign}`,
                            timestamp: serverTimestamp()
                        });

                        // Local state update
                        if(!State.alliance.managers) State.alliance.managers = [];
                        State.alliance.managers.push(uid);

                        App.ui.toast("Member Promoted");
                        App.renderTableOnly(); // Refresh buttons
                    } catch(e) { console.error(e); App.ui.toast("Error promoting member", "error"); }
                },

                demoteMember: async (uid) => {
                    if (!confirm(`Demote to Member?`)) return;
                    try {
                        await updateDoc(doc(App.db, 'alliances', State.alliance.id), {
                            managers: arrayRemove(uid)
                        });
                        ...
                        await addDoc(collection(App.db, 'alliances', State.alliance.id, 'logs'), {
                         ...
                         // Local state update
                        State.alliance.managers = State.alliance.managers.filter(id => id !== uid);

                        App.ui.toast("Member Demoted");
                        App.renderTableOnly();
                    } catch(e) { console.error(e); App.ui.toast("Error demoting member", "error"); }
                },
                
                // NEW: Delete Profile (GDPR)
                deleteProfile: async () => {
                    if(!confirm("WARNING: This will permanently delete your profile, stats, and history. This cannot be undone. Are you sure?")) return;
                    try {
                        // Delete Profile Doc
                        await deleteDoc(doc(App.db, 'profiles', State.user.uid));
                        // Delete User Doc
                        await deleteDoc(doc(App.db, 'users', State.user.uid));
                        // Logout
                ...
                compareSelected: () => {
                    ...
                    // FIXED: USE VIEWMODE
                    const players = (State.viewMode === 'global' ? State.data.global : State.data.roster).filter(p => selectedIds.includes(p.id));
                    // FIXED: FILTER PRIVACY
                    const visiblePlayers = players.filter(p => {
                        return !p.privacy?.hideStats || App.permissions.canViewHidden(p);
                    });

                    if (visiblePlayers.length < 2) return App.ui.toast("Selected players have hidden stats", "error");
                    ...
                    new Chart(document.getElementById('chart-compare'), {
                        ...
                        data: {
                            ...
                            datasets: visiblePlayers.map((p, i) => {
                ...
                saveProfile: async () => {
                    ...
                    // DUPLICATE CHECKS
                    const currentId = State.editorTarget || State.user.uid;
                    const pidQ = query(collection(App.db, 'profiles'), where('pid', '==', pid));
                    const pidSnap = await getDocs(pidQ);
                    if(!pidSnap.empty && pidSnap.docs[0].id !== currentId) return App.ui.toast("PID already registered", "error");

                    const ignQ = query(collection(App.db, 'profiles'), where('ign', '==', ign), where('kingdom', '==', parseInt(kingdom)));
                    const ignSnap = await getDocs(ignQ);
                    if(!ignSnap.empty && ignSnap.docs[0].id !== currentId) return App.ui.toast("IGN exists in this Kingdom", "error");
                    ...
                    const profileData = { 
                        ign, 
                        kingdom: parseInt(kingdom), // FORCE NUMBER
                        pid, 
                    ...
                    try {
                        const batch = writeBatch(App.db);
                        const pRef = doc(App.db, 'profiles', State.editorTarget || State.user.uid);
                ...
                // NEW: Admin Functions
                ownerToggleGlobal: async () => {
                    const newState = !State.config.hideGlobal;
                    await setDoc(doc(App.db, 'metadata', 'security'), { hideGlobal: newState }, { merge: true });
                    App.ui.toast("Global Visibility Updated");
                },
                setCreationCode: async () => {
                    const code = document.getElementById('admin-creation-code').value.trim();
                    await setDoc(doc(App.db, 'metadata', 'security'), { creationCode: code }, { merge: true });
                    App.ui.toast("Creation Code Updated");
                },
                adminSearchUser: async () => {
                     const ign = document.getElementById('admin-search').value.trim();
                     if (!ign) return;
                     const q = query(collection(App.db, 'profiles'), where('ign', '==', ign));
                     const snap = await getDocs(q);
                     if (snap.empty) return App.ui.toast("User not found", "error");
                     // Open Editor for first match
                     const target = snap.docs[0];
                     App.actions.editPlayer(target.id);
                },

                // NEW: Open Editor for specific user (Admin/Manager)
                editPlayer: async (uid) => {
                    try {
                        const snap = await getDoc(doc(App.db, 'profiles', uid));
                        ...
                        State.editorTarget = uid; // Set Target
                        State.form = { 
                            ign: raw.ign, kingdom: raw.kingdom, pid: raw.pid, 
                            privacy: raw.privacy || {hideName: false, hideStats: false}, 
                            troops: { inf: normalize(raw.troops?.inf), cav: normalize(raw.troops?.cav), arc: normalize(raw.troops?.arc) }, 
                            stats: s 
                        };
                        
                        State.current = 'editor';
                        App.render();
                    } catch(e) { App.ui.toast("Error loading player", "error"); }
                },
        ...
        window.ownerToggleGlobal = () => App.actions.ownerToggleGlobal();
        window.setCreationCode = () => App.actions.setCreationCode();
        window.adminSearchUser = () => App.actions.adminSearchUser();
        ...
        window.deleteProfile = () => App.actions.deleteProfile();
